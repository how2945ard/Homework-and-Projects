    ...
    for (i = 0; i <= fdmax; i++) {
      if (FD_ISSET(i, &read_fds)) { // we got one!!
        if (i == svr.listen_fd) {
          ...
          int current_socket = socket_max - socket_count;
          ...
          // FD_CLR(svr.listen_fd, &master); // remove from master set
          // FD_SET(conn_fd, &master); // add to master set
          // if (conn_fd > fdmax) {    // keep track of the max
          //     fdmax = conn_fd;
          // }
